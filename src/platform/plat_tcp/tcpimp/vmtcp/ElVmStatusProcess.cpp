...
ElVmStatusProcess::ElVmStatusProcess()
{
    ...
    m_tcpwork      = gVisitorMonitorLoginTcpAgent;
...
int ElVmStatusProcess::AlloceTcpImp()
{
    ...
    do {
        if (m_vmtcpimplist.size() > m_currentTcp)
        {
			...
			int delflag = 0;
            for each (TcpImp * imp in m_vmtcpimplist)
            {
                if (!imp->Effective()) {
                    //imp->deleteLater();
...
TcpImp *ElVmStatusProcess::CurrentTcpVMImp()
{
...
{
    TcpImp *tcpimp = (TcpImp *)QObject::sender();
