...
TcpLoginAgent::TcpLoginAgent() : QObject()
{
...
TcpLoginAgent *TcpLoginAgent::m_instance = nullptr;


TcpLoginAgent::~TcpLoginAgent()
{

}

void TcpLoginAgent::Init(QString loginurl, QString port)
{
...
void TcpLoginAgent::CreateTcpConnect(QString loginurl, QString port)
{
    m_isConnect = false;
    m_loginUrl  = loginurl;
    m_port      = port;
...
void TcpLoginAgent::Clear()
{

}

void TcpLoginAgent::TcpReconnect()
{
...
TcpLoginAgent *TcpLoginAgent::GetInstance()
{
    if (m_instance == nullptr)
    {
        m_instance = new TcpLoginAgent;
    }
    return m_instance;
}


void TcpLoginAgent::R_SetCurrentLoginStatus(ELOGINSTATUS oldState, ELOGINSTATUS newState)
{
...
void TcpLoginAgent::DoExecLoginCmd(const QString &userName, const QString &password, int flag,int isForce)
{
    m_loginType = flag;
...
void TcpLoginAgent::DoExecOfflineMessageCmd()
{
...
void TcpLoginAgent::DoUpdateLoginToken(const QString& token)
{

}


void TcpLoginAgent::R_connected()
{
...
void TcpLoginAgent::R_MessageParsingFailed()
{
    Q_EMIT S_MessageParsingFailed();
}

void TcpLoginAgent::R_ReceivedMessage(TcpMessagePtr msg)
{
...
void TcpLoginAgent::R_NetWorkError(int errorCode, QString msg)
{
...
void TcpLoginAgent::ReTryLoginConnect()
{
    m_setRetryLogin = 0;
    if (m_ForceLogout == false)
        CreateTcpConnect(m_loginUrl, m_port);
...
void TcpLoginAgent::R_TokenError()
{
    Q_EMIT S_NetWorkError(400, "token error");
}


void TcpLoginAgent::R_ReloginTimeOut()
{
    if (m_reLoginFlag)
    {
        m_reLoginFlag = 0;
...
void TcpLoginAgent::R_DoExecLoginCmdResult(TcpMessagePtr msg)
{
