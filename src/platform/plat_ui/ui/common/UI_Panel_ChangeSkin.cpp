...
UI_Panel_ChangeSkin::UI_Panel_ChangeSkin(QWidget *parent) : QWidget(parent), m_CurBtn(nullptr)
{
    ...
    connect(CommonSignalService::Instance(), SIGNAL(S_CurrentSkinChanged()), this, SLOT(R_ResetCurrentSkin()));
}
...
{
    QMap<QString, UISkinInfo *> mapSkin = gUISystemConfigManager->GetSkinMap();
...
void UI_Panel_ChangeSkin::SetDefaultSkin(QString strName)
{
    if (m_mapSkinBtnByImgName.contains("bg_PureColor_1"))
    {
        ...
        if (btn)
        {
            btn->SetCheck(true);
            btn->update();
            gUISystemConfigManager->CurrentSkin(btn->GetSkinInfo());
...
void UI_Panel_ChangeSkin::R_ResetCurrentSkin()
{
    ...
    m_pCurSkinInfo = gUISystemConfigManager->CurrentSkin();
