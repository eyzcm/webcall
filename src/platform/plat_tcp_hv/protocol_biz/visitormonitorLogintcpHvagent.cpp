#include "VisitorMonitorLoginTcpHvAgent.h"
...
VisitorMonitorLoginTcpHvAgent::VisitorMonitorLoginTcpHvAgent() : ITcpWork()
{
    m_logincount           = 0;
    m_isVmLogined          = false;
    m_isConnect            = false;
...
VisitorMonitorLoginTcpHvAgent* VisitorMonitorLoginTcpHvAgent::m_instance = nullptr;

void VisitorMonitorLoginTcpHvAgent::R_TcpConnect()
{
}

void VisitorMonitorLoginTcpHvAgent::R_Login()
{
    DoExecVMAuthLogin();
}

void VisitorMonitorLoginTcpHvAgent::Init(QString loginurl, QString port, QString userName, QString companyId, QString monitorvisitornumber)
{
...
void VisitorMonitorLoginTcpHvAgent::Clear()
{
    m_isLastAddressInfo = false;
...
void VisitorMonitorLoginTcpHvAgent::TcpReconnect(QString loginurl, QString port)
{
    m_isConnect   = false;
    m_isLogin     = false;
    m_isVmLogined = false;
...
QString VisitorMonitorLoginTcpHvAgent::Key() const
{
    return m_vmKey;
}

void VisitorMonitorLoginTcpHvAgent::DoExecVMLogin(QString userId, QString companyId)
{
    m_isLogin   = true;
    m_userName  = userId;
    m_companyId = companyId;
...
void VisitorMonitorLoginTcpHvAgent::DoExecVMAuthLogin()
{
    //	if (m_isConnect)
    {
...
void VisitorMonitorLoginTcpHvAgent::DoExecVMKeepAlive()
{
...
void VisitorMonitorLoginTcpHvAgent::DoExecVMGetVisitorList()
{
...
VisitorMonitorLoginTcpHvAgent* VisitorMonitorLoginTcpHvAgent::GetInstance()
{
    if (m_instance == nullptr)
    {
        m_instance = new VisitorMonitorLoginTcpHvAgent;
    }
    return m_instance;
}

VisitorMonitorLoginTcpHvAgent::~VisitorMonitorLoginTcpHvAgent()
{
    m_isVmLogined = false;
    m_loginUrl    = "";
    m_port        = "";
...
void VisitorMonitorLoginTcpHvAgent::R_SetCurrentLoginStatus(ELOGINSTATUS oldState, ELOGINSTATUS newState)
{
...
void VisitorMonitorLoginTcpHvAgent::R_ReceivedMessage(TcpMessagePtr pmsg)
{
...
void VisitorMonitorLoginTcpHvAgent::R_NetWorkError(int errorCode, QString msg)
{
...
void VisitorMonitorLoginTcpHvAgent::R_DoExecVMLoginCmdResult(TcpMessagePtr msg)
{
...
void VisitorMonitorLoginTcpHvAgent::R_DoExecVMAuthLoginCmdResult(TcpMessagePtr msg)
{
    Json::Value val;
    bool        isOK = JsonHelper::GetJsonValueFromBody(msg->Body(), val);
    if (isOK)
...
void VisitorMonitorLoginTcpHvAgent::R_connected()
{
    m_retryVMLoginCount = 0;
    m_isConnect         = true;
    if (m_isVmLogined)
...
void VisitorMonitorLoginTcpHvAgent::R_MessageParsingFailed()
{
    Q_EMIT S_MessageParsingFailed();
}

void VisitorMonitorLoginTcpHvAgent::ReTryVMLoginConnect()
{
    m_setRetryLogin = 0;
...
void VisitorMonitorLoginTcpHvAgent::DoTryVMLoginConnect()
{
    TcpReconnect(m_loginUrl, m_port);
}

void VisitorMonitorLoginTcpHvAgent::ParseAuth(TcpMessage* msg)
{
    if (m_isVmLogined)
    {
        //已经获取到key
...
void VisitorMonitorLoginTcpHvAgent::DoExecVMAuthLoginCmdResult(TcpMessagePtr message)
{
    return R_DoExecVMAuthLoginCmdResult(message);
}

bool VisitorMonitorLoginTcpHvAgent::isLoginMaxCount()
{
    return (m_logincount >= 10);
}

void VisitorMonitorLoginTcpHvAgent::R_test1()
{
    qDebug()<<"r_test1";
    int i = 0;
}
