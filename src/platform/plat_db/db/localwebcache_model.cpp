...
QString LocalWebCache_Model::m_tableName = "yl_webcache";
CACHE_STATUS   LocalWebCache_Model::m_status = S_NONE;
QSqlDatabase LocalWebCache_Model::m_db;


LocalWebCache_Model::LocalWebCache_Model()
{

}

void LocalWebCache_Model::LoadDb( QSqlDatabase db )
{
    m_db = db;
    if( !m_db.open()) return ;
    m_status = S_OPEN;
...
bool LocalWebCache_Model::CreateTable(void)
{
    // 用于执行sql语句的对象
...
// 判断数据库中数据表是否存在
bool LocalWebCache_Model::IsTableExist()
{
...
bool LocalWebCache_Model::Insert()
{
    QSqlQuery sqlQuery(m_db);
...
bool LocalWebCache_Model::Update()
{
...
// 删除数据
bool LocalWebCache_Model::DeleteData(QString name )
{
    QSqlQuery sqlQuery(m_db);
...
LocalCache * LocalWebCache_Model::QueryListByName(const QString name)
{
...
QList<LocalCache *> LocalWebCache_Model::QueryList(const QString where)
{
...
bool LocalWebCache_Model::HasRecord()
{
    QSqlQuery sqlQuery(m_db);
...
bool LocalWebCache_Model::DeleteAllMemData()
{
    {
        QSqlQuery sqlQuery(m_db);
...
QList<LocalCache *> LocalWebCache_Model::QueryInitList()
{
    QList<LocalCache *> list;
    QSqlQuery sqlQuery(m_db);
...
bool LocalWebCache_Model::DeleteDataByFiter(QString name )
{
    QSqlQuery sqlQuery(m_db);

    sqlQuery.exec(QString("DELETE FROM "+m_tableName+" WHERE name like  '%1'").arg(name));
    if(!sqlQuery.exec())
    {
...
bool LocalWebCache_Model::ClearData()
{

    QSqlQuery sqlQuery(m_db);

    sqlQuery.exec(QString("DELETE FROM "+m_tableName));
    if(!sqlQuery.exec())
    {
