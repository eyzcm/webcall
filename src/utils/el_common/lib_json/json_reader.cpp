...
namespace Json
{
    ...
    bool Reader::decodeDouble(Token &token)
    {
	  ...
	  {
		  count = sscanf( buffer, "%lf", &value );
		  QString tt3 =QString::number(value);
		  //int tt2 = value%10;
		  double tbss = value/100;
		  double tbss1 = value - tbss*100;

		  Char tt[32];
		  strncpy( tt , buffer , 16);
		  double hvalue = 0;
		  double lvalue = 0;
		  count = sscanf( tt, "%lf", &hvalue );
		  tt1 = hvalue;
		  count = sscanf( buffer+16, "%lf", &lvalue );
    ...
    bool Reader::decodeULong(Token &token)
    {
        ULONG64   value      = 0;
        ...
        Char buffer[bufferSize];
        memcpy(buffer, token.start_, length);
        if ((length == bufferSize) && ((buffer[0] > '1') || (buffer[1] > '7'))) //大于 18446744071132575792
            return false;
        double tvalue  = 0;
        double thvalue = 0;
        buffer[length] = 0;
        if (length < 16)
        {
            count = sscanf(buffer, "%lf", &tvalue);
            value = tvalue;
        }
        else
        {
            char * endbuf = NULL;
            value = std::strtoull(buffer,&endbuf,10);
            if( endbuf ) count = 1;
