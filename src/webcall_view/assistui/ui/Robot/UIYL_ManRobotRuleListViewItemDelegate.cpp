...
UIYL_ManRobotRuleListViewItemDelegate::UIYL_ManRobotRuleListViewItemDelegate(QObject *parent) : QStyledItemDelegate(parent), m_pButton(new QPushButton()), m_nSpacing(4), m_nWidth(15), m_nHeight(15)
{
    // 设置按钮正常、划过、按下样式
...
UIYL_ManRobotRuleListViewItemDelegate::~UIYL_ManRobotRuleListViewItemDelegate()
{
    if (m_pButton)
    {
...
QSize UIYL_ManRobotRuleListViewItemDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    return QSize(0, 30);
}

bool UIYL_ManRobotRuleListViewItemDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    m_nType             = -1;
...
void UIYL_ManRobotRuleListViewItemDelegate::DoDrawPixmap(QPainter *painter, const QStyleOptionViewItem &option, QRect rect, QPixmap &pixmap, QString pixname) const
{
...
void UIYL_ManRobotRuleListViewItemDelegate::DoDrawText(QPainter *painter, const QStyleOptionViewItem &option, QRect rect, QString text) const
{
    QStyleOptionViewItem tempOption = option;
...
void UIYL_ManRobotRuleListViewItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
void UIYL_ManRobotRuleListViewItemDelegate::DoDrawManRobotRuleLeafNode(YL_ManRobotRuleListItem *item, YL_ManRobotRuleLeafNodeItemData *nodeItem, const QStyleOptionViewItem &option, QPainter *painter) const
{
    if (nodeItem)
    {
...
void UIYL_ManRobotRuleListViewItemDelegate::DoDrawNameText(QPainter *painter, const QStyleOptionViewItem &option, QRect rect, QString text) const
{
    QStyleOptionViewItem tempOption = option;
...
QStringList UIYL_ManRobotRuleListViewItemDelegate::GetButtonListByData(YL_ManRobotRuleLeafNodeItemData *data) const
{
    QStringList retList;
    if (data == nullptr)
    {
...
void UIYL_ManRobotRuleListViewItemDelegate::paintButton(QPainter *painter, const QStyleOptionViewItem &option, YL_ManRobotRuleLeafNodeItemData *leafData) const
{
    QStringList buttonList = GetButtonListByData(leafData);
    // 计算按钮显示区域
    int nCount = buttonList.count();
...
YL_ManRobotRuleListItem *UIYL_ManRobotRuleListViewItemDelegate::ItemFromIndex(const QModelIndex &index) const
{
    if (!index.isValid())
        return NULL;
